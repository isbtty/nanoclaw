...
> daemon 接続情報を直接読みます (host の `WorkingDirectory` は
...
> **【2026-08 更新】キー名は `BOSWELL_DAEMON_*` が正 (ADR-0036 のリネーム)。**
> 旧 `DESHI_DAEMON_*` も互換で読みます (新名優先)。解決順は
> [`src/deshi/daemon-env.ts`](../../src/deshi/daemon-env.ts) に集約。
> plist に書き込むキー名は旧名のままですが、読み口が両対応なので実害はありません。
...
2. `.env` から `BOSWELL_DAEMON_URL` / `BOSWELL_DAEMON_DEVICE_SECRET` (無ければ旧 `DESHI_DAEMON_*`) を読む
...
`.env` にどちらの名前も無い場合は exit 1 + ヒント表示で止まる
...
- [`/boswell-add-host-tools` skill (deshi repo)](https://github.com/isbtty/deshi/blob/main/.claude/skills/boswell-add-host-tools/SKILL.md) — device secret を `.env` に書く前段ステップ
