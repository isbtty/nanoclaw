# ADR-0021: 権限分離は BOT の物理分離で行う — 知識検索BOT / 管理者BOT の 2 インスタンス構成
...
| 7 | **container は host-tools-server を curl で直接叩ける**。`TOOL_ALLOWLIST` (`container/agent-runner/src/providers/claude.ts:44`) は agent group に依らず `Bash` を含み、`/tools/*` は loopback 無認証 (`src/deshi/host-tools-server.ts`)、`PROXY_BYPASS` (`src/container-runner.ts:509`) は `host.docker.internal` を素通しにする。よって §4 の profile 絞り込みは **MCP 経由の口を塞ぐだけで、経路そのものは塞げていない**。prompt injection された知識検索 container が `deshi_daemon_run_skill` に任意の `channelContext` を渡せば、**本 ADR が防ごうとしている scope 偽造がそのまま成立する** (`deshi_daemon_send_file_to_chat` に任意 path を渡す知識ベース流出も同様) | **未受容。base → main の前に判断が要る** | `/tools/*` に呼び出し元の identity を要求するか、egress proxy で path を絞るか |
