...
- `/boswell-add-slack-permission-split` — `/add-slack` のオプションとして、管理者BOT と知識検索BOT の 2 本立てを配線する初回セットアップ。当初 `/boswell-setup-permission-split` で起票したが、add-slack の派生である位置づけを名前に出すため改名した (ADR-0016 の許可動詞 `add` を使用)。
  実行 FB を反映: (1) 管理者BOT の不足スコープはチェックボックスではなく App Manifest の
  貼り替えで直す手順にした (手作業だと保存漏れ・再インストール未完了で 2 回失敗した)。
  `auth.test` のレスポンスヘッダでスコープを確認するコマンドも載せた。
  (2) 知識検索BOT の表示名を `AskUserQuestion` で人間に決めさせる手順を追加 (旧: "Knowledge Search" 固定)。(3) `/add-slack` に、権限分離を予定しているなら最初から
  マニフェストで作るよう促す deshi ブロックを追記。
